...
- 云备份开启后会自动同步：打开时自动从云端拉取并合并，保存账单后约 3 秒自动上传；失败时保留本机数据并在联网、重新打开或稍后自动重试。“我的 → 云同步”会显示最近成功时间或待同步原因。
- 新版云备份用手机号识别同一个账号，PIN 只用于认证和解密；同一手机号输入不同 PIN 会提示同步密码不匹配，不会再变成另一套账号。
- 主屏幕 Web App 打开时优先使用本机缓存立即显示，再在后台检查网页新版本；“我的 → 当前版本”仍可手动刷新。无网时继续保留当前缓存，不会清空本机账单。
- 账单页只保留流水与日历内容，不重复展示首页已有的收支摘要；主屏幕四个板块支持明显的左右滑动切换，轻微横移、上下滚动和图表/控件操作不会误切页。
- 统计页分为“趋势、排行、月报”：趋势查看每日支出，排行可在分类行内展开明细，月报默认定位本月并用年度折线查看各月支出，再展示环比变化、活跃日均、分类占比和大额支出。

...
重要：iPhone 的“主屏幕独立 Web App”和 Safari 可能是两套本地存储。当前没有自动云同步，但已经支持 JSON 导出与导入；如果要使用“双击辅助触控 → 快捷指令 → 快速记账”，建议整个应用都在 Safari 模式使用：添加到主屏幕时关闭“作为 Web App 打开”（如果你的 iOS 显示这个开关），这样日常打开和快捷指令会落在同一个 Safari 数据空间。若选择独立 Web App 模式，则先只从主屏幕图标进入，不要把快捷指令保存的记录当作主屏幕 App 的账单。
